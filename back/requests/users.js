...
export const updatePassword = async (newPassword, currentPassword) => {
    try {
        const [result] = await pool.query(
            "UPDATE users SET password = ? WHERE password = ?", [newPassword, currentPassword]
        );
        if (result.length === 0) {
            return result[0] = -1;
        } else {
        ...
        createError(1001, "Problème a l'update du nouveau mdp pour l'utilisateur correspondant au mdp : ", currentPassword);
    }
};
